...
public class SuccessFactorsPluginConfigTest {
  ...
  @Test
  public void testInvalidRowSkipCount() {
    SuccessFactorsPluginConfig pluginConfig = pluginConfigBuilder
      .skipRowCount(-2L)
      .build();
    try {
      pluginConfig.validatePluginParameters(failureCollector);
      Assert.fail("Rows to skip are less than 0");
    } catch (ValidationException ve) {
      List<ValidationFailure> failures = ve.getFailures();
      Assert.assertEquals(1, failures.size());
      Assert.assertEquals(ResourceConstants.ERR_NEGATIVE_PARAM_PREFIX
                            .getMsgForKey("Number of Rows to Skip"), failures.get(0).getMessage());
    }
  }

  @Test
  public void testInvalidSplitCount() {
    SuccessFactorsPluginConfig pluginConfig = pluginConfigBuilder
      .splitCount(-2)
      .build();
    try {
      pluginConfig.validatePluginParameters(failureCollector);
      Assert.fail("Split count is less than 0");
    } catch (ValidationException ve) {
      List<ValidationFailure> failures = ve.getFailures();
      Assert.assertEquals(1, failures.size());
      Assert.assertEquals(ResourceConstants.ERR_NEGATIVE_PARAM_PREFIX
  ...
  public void testInvalidBatchSize() {
    ...
    try {
      pluginConfig.validatePluginParameters(failureCollector);
      Assert.fail("Batch Size is less than 0");
    } catch (ValidationException ve) {
      List<ValidationFailure> failures = ve.getFailures();
      Assert.assertEquals(1, failures.size());
      Assert.assertEquals(ResourceConstants.ERR_NEGATIVE_PARAM_PREFIX
                            .getMsgForKey("Batch Size"), failures.get(0).getMessage());
    }
  }

  @Test
  public void testInvalidRowToFetch() {
    SuccessFactorsPluginConfig pluginConfig = pluginConfigBuilder
      .numRowsToFetch(-1L)
      .build();
    try {
      pluginConfig.validatePluginParameters(failureCollector);
      Assert.fail("Rows to fetch are less than 0");
    } catch (ValidationException ve) {
      List<ValidationFailure> failures = ve.getFailures();
      Assert.assertEquals(1, failures.size());
      Assert.assertEquals(ResourceConstants.ERR_NEGATIVE_PARAM_PREFIX
                            .getMsgForKey("Number of Rows to Fetch"), failures.get(0).getMessage());
    }
  }
