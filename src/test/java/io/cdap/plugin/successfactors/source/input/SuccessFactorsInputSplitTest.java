...
public class SuccessFactorsInputSplitTest {
  ...
  public void testReadFields() throws IOException {
    ...
    Mockito.when(objectInputStream.readLong()).thenReturn(1L);
    successFactorsInputSplit.readFields(objectInputStream);
    Assert.assertEquals(1L, successFactorsInputSplit.getStart());
    Assert.assertEquals(1L, successFactorsInputSplit.getEnd());
    Assert.assertEquals(1L, successFactorsInputSplit.getBatchSize());
  }
  ...
  public void testWrite() throws IOException {
    ...
    Assert.assertEquals(1L, successFactorsInputSplit.getStart());
    Assert.assertEquals(2L, successFactorsInputSplit.getEnd());
    Assert.assertEquals(20L, successFactorsInputSplit.getBatchSize());
  }
